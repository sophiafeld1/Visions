...
const { getProductById } = require("./products-store");
const { getQuantity, reserveItems, releaseItems } = require("./inventory-store");
...
async function createCheckoutSession({ items, origin }) {
  ...
  await reserveItems(reservationItems);

  let session;
  try {
    session = await stripe.checkout.sessions.create(sessionConfig);
  } catch (error) {
    await releaseItems(reservationItems);
    throw error;
  }
