...
const { loadProductsFromFile } = require("./products-store");

const LIVE_INVENTORY_FILE = path.join(__dirname, "..", "data", "inventory-live.json");
...
function readFileState() {
  if (!fs.existsSync(LIVE_INVENTORY_FILE)) {
    return {};
  }

  return JSON.parse(fs.readFileSync(LIVE_INVENTORY_FILE, "utf8"));
}

function writeFileState(state) {
  fs.writeFileSync(LIVE_INVENTORY_FILE, `${JSON.stringify(state, null, 2)}\n`);
}
...
async function releaseCheckoutSession(sessionId, items) {
  ...
  await releaseItems(items);
  state.__releasedSessions[sessionId] = true;
  writeFileState(state);
}
